...
export default function AddChart() {
  ...
  const fetchAddStats = useCallback(async () => {
    try {
      setLoading(true);
      const response = await profileService.getAddStats(period);
      ...
      console.error('Error fetching add stats:', err);
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-gray-50 rounded-lg">
          <div>
            ...
            <p className="text-lg font-bold text-gray-800">{stats.total} cards</p>
          </div>
          <div>
            <p className="text-sm text-gray-600">Average</p>
            <p className="text-lg font-bold text-gray-800">{stats.average} cards/day</p>
          </div>
          <div>
            <p className="text-sm text-gray-600">Period</p>
            <p className="text-lg font-bold text-gray-800">{stats.period}</p>
          </div>
        </div>
      )}
    </div>
